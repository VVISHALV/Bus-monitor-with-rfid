...
var {bus, bus}=require('../models/bus');
...
router.get('/',(req,res)=>{
    bus.find((err,docs)=>{
        ...
        {console.log('Error in retrieving bus'+JSON.stringify(err,undefined,2));}
    });
});
...
var timer=today.getHours() + ":" + today.getMinutes() + ":" + today.getSeconds();
    ...
    buspost.save((err,doc)=>{
        if(!err){res.send(doc);}
        else{console.log('Error in bus post'+JSON.stringify(err,undefined,2));}
    });
});
